...
REPORT Z_MATRIXMULT.
DATA:
      lv_m              TYPE i VALUE 1440,
      lv_n              TYPE i VALUE 1440,
      lv_task_no        TYPE i,
      lv_task_no_str    TYPE string,
      ...
      lo_matrix_c_multi TYPE REF TO zcl_matrix.
...
LOOP AT lo_matrix_c->matrix ASSIGNING <fs_line_c>.
  LOOP AT lo_matrix_a->matrix ASSIGNING <fs_line_a> USING KEY m_sort WHERE m = <fs_line_c>-m.
    READ TABLE lo_matrix_b->matrix WITH KEY mn_unique COMPONENTS m = <fs_line_a>-n n = <fs_line_c>-n ASSIGNING <fs_line_b>.
    <fs_line_c>-value = <fs_line_c>-value + ( <fs_line_a>-value * <fs_line_b>-value ).
  ENDLOOP.
ENDLOOP.

GET RUN TIME FIELD lv_start_threaded.
lv_difference = CONV decfloat34( ( lv_start_threaded - lv_start_regular ) / 1000000 ).
